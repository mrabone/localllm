...
Sessions are stored in a JSON registry at `~/.localllm_sessions.json` (configurable via `SESSIONS_REGISTRY`). The registry maps human-readable session names to UUIDs:
